...
?>
   ...
   <?php
        ...
        $lblGroupACP    = Helper::cmsLinkSort('Group ACP', 'group_acp', $column, $column_dir);
        ...
        $selectboxStatus        = Helper::cmsSelectbox("filter_state", "inputbox", array(2 => "- Select Status -", 1 => "Publish", 0 => "Unpublish"), $selectedStatus);
         ...
         //SELECT ACP SORT
        $selectedACP            = isset($this->arrParams["filter_acp"])? $this->arrParams["filter_acp"] : 2;  
        $selectboxACP        = Helper::cmsSelectbox("filter_acp", "inputbox", array(2 => "- Select ACP -", 1 => "Yes", 0 => "No"), $selectedACP);    
                        ...
                        <?php
                            if(!empty($this->_listItem)){
                                ...
                                foreach ($this->_listItem as $item) {
                                    ...
                                    $status     = Helper::cmsStatus($item["status"], URL::createLink("admin", "group", "ajaxStatus", array("status" => $item["status"], "id" => $id)), $id);
                                    $group_acp  = Helper::cmsACP($item["group_acp"], URL::createLink("admin", "group", "ajaxACP", array("group_acp" => $item["group_acp"], "id" => $id)), $id);
