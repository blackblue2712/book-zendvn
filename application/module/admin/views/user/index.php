   ...
   <?php
        ...
        $lblUserName    = Helper::cmsLinkSort("Username", "username", $column, $column_dir);
        $lblEmail       = Helper::cmsLinkSort("Email", "email", $column, $column_dir);
        $lblFullName    = Helper::cmsLinkSort("Full name", "fullname", $column, $column_dir);
        $lblStatus      = Helper::cmsLinkSort("Status", "status", $column, $column_dir);
        $lblGroup       = Helper::cmsLinkSort('Group', 'group_id', $column, $column_dir);
        ...
        $selectboxStatus        = Helper::cmsSelectbox("filter_state", "inputbox", array(2 => "- Select Status -", 1 => "Publish", 0 => "Unpublish"), $selectedStatus);
         ...
         //SELECT ACP SORT
        $selectedGroup       = isset($this->arrParams["filter_group"])? $this->arrParams["filter_group"] : "default";  
        $selectboxGroup      = Helper::cmsSelectbox("filter_group", "inputbox", $this->_slbGroup , $selectedGroup);    
                        ...
                        <?php
                            if(!empty($this->_listItem)){
                                ...
                                foreach ($this->_listItem as $item) {
                                    ...
                                    $status     = Helper::cmsStatus($item["status"], URL::createLink("admin", "user", "ajaxStatus", array("status" => $item["status"], "id" => $id)), $id);
