<?php
	class URL{
		public static function createLink($mod, $controller, $action, $params=null, $router = null){
			if($router != null){
			...
			$url =  '/bookstore/index.php?mod='.$mod.'&controller='.$controller.'&action='.$action . $linkParams;
			return $url;
		}

		public static function redirect($mod, $controller, $action, $params=null, $router = null){
			$link = self::createLink($mod, $controller, $action, $params, $router);
			header("location: " . $link);
			exit();
		}

		public static function checkRefreshPage($value, $mod, $controller, $action, $option = null){
			if($option == null){
				if( Session::get("tooken") == $value){
					Session::delete("tooken");
					URL::redirect($mod, $controller, $action);
				}else{
					Session::set("tooken", $value);
				}
			}
