...
class Validate{
			...
			if($value['required'] == true && trim($this->source[$element])==null){
				...
				switch ($value['type']) {
					case 'int':
						...
						break;
					case 'intI0':
						$this->validateIntI0($element, $value['options']['min'], $value['options']['max']);
						break;
					case 'string':
	...
	// Validate Integer include 0
	private function validateIntI0($element, $min = 0, $max = 0){
		if(!is_numeric($this->source[$element]) || $this->source[$element]<$min || $this->source[$element]>$max){
			$this->setError($element, 'is an invalid number');
		}
	}
	...
	public function showErrors(){
		...
		if(!empty($this->errors)){
			$xhtml .= '<dl id="system-message" <dt="" class="error">error<dd class="error message"><ul>';
	...
	private function validateStatus($element, $option){
		...
		if(in_array($this->source[$element], $option["deny"]) == true){
			$this->setError($element, 'Vui lòng không chọn giá trị mặc định!');
		}
	}
